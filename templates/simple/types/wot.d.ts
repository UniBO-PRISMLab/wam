...
declare namespace WoT {
   ...
   */
  function discover(filter?: ThingFilter): ThingDiscovery;

  /**
   * Accepts a ThingDescription and returns a ConsumedThing
   * @param td thing description
   */
  function consume(td: ThingDescription): Promise<ConsumedThing>;
   ...
   * @param td thing description
   */
  function produce(td: ThingDescription): Promise<ExposedThing>;
   ...
   */
  type ThingDescription = { [key: string]: any };
  ...
  type PropertyValueMap = object | { [key: string]: any };

  type WotListener = (data: any) => void;
  ...
  type PropertyReadHandler = (options?: InteractionOptions) => Promise<any>;

  type PropertyWriteHandler = (value: any, options?: InteractionOptions) => Promise<any>;

  type ActionHandler = (params: any, options?: InteractionOptions) => Promise<any>;
}
